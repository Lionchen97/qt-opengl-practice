...
void OpenGlWidget::initializeGL()
{
    ...
    // 1.通常方式
    // 告诉GPU如何解析缓冲里的属性值 从2开始，每个点有3个属性值，不需要标准化，步长为一个点的大小，
     glVertexAttribPointer(2,3,GL_FLOAT,GL_FALSE,3*sizeof(float),(void*)0); // 在顶点着色器声明layout时调用
    // 2.询问的方式获取layout
    // A.OpenGL原生
    // GLuint pos_layout = glGetAttribLocation(shaderProgram,"aPos");
    // B.Qt封装方式 需要线设置着色器程序
    // shaderProgram.bind();
    // GLint layout = shaderProgram.attributeLocation("aPos");
    // glVertexAttribPointer(layout,3,GL_FLOAT,GL_FALSE,3*sizeof(float),(void*)0);

    // 开启VAO管理的第lay_out个属性值
